...
class CAppUI {
    ...
    /** @var boolean Defines if user is associated to a client company */
    var $fullaccess = null;
	...
	function setUserLocale($loc='', $set = true) {
		...
		list($base_locale,$english_string,   $default_language, $lcs) = $lang;
		if (! isset($lcs)) {
   ...
   function __($str, $flags = 0) {
    ...
    // Traducción (sin @ para no ocultar errores)
    if (isset($GLOBALS['translate'][$str]) && $GLOBALS['translate'][$str] !== '') {
        $str = $GLOBALS['translate'][$str];
    } else if (function_exists('dPgetConfig') && dPgetConfig('locale_warn')) {
        if ($this->base_locale != $this->user_locale ||
            ($this->base_locale == $this->user_locale && (!isset($GLOBALS['translate'][$str])))) {
            $alert = dPgetConfig('locale_alert');
            if (!empty($alert)) {
                $str .= $alert;
            }
        }
    ...
    switch ($flags & UI_CASE_MASK) {
        ...
        case UI_CASE_UPPERFIRST:
            // ucwords no es multibyte; usamos mb_convert_case
            $str = mb_convert_case($str, MB_CASE_TITLE, 'UTF-8');
            break;
    }

    // Charset/encoding seguro
    global $locale_char_set;
    $encoding = $locale_char_set ?: (function_exists('dPgetConfig') ? dPgetConfig('locale_char_set') : null);
    if (!$encoding) {
        $encoding = 'UTF-8';
    }

    // Algunos proyectos antiguos guardan "es_co" aquí por error. Corrigimos:
    $encoding = trim($encoding);
    $lower = strtolower($encoding);
    // Lista corta de codificaciones válidas comunes en entornos hispanos
    $knownEncodings = ['utf-8', 'iso-8859-1', 'windows-1252'];
    if (!in_array($lower, $knownEncodings, true)) {
        // Si llega algo tipo "es_co", “es-CO”, etc., forzamos a UTF-8
        $encoding = 'UTF-8';
    } else {
        // Normaliza capitalización (htmlentities espera “UTF-8”, “ISO-8859-1”, etc.)
        $encoding = strtoupper($lower) === 'UTF-8' ? 'UTF-8' :
                    (strtoupper($lower) === 'ISO-8859-1' ? 'ISO-8859-1' : 'Windows-1252');
    }

    // Salida según flags
    switch ($flags & UI_OUTPUT_MASK) {
        case UI_OUTPUT_HTML:
            // ENT_SUBSTITUTE evita warnings por bytes inválidos; ENT_COMPAT mantiene compatibilidad de comillas
            $str = htmlentities($str, ENT_COMPAT | ENT_SUBSTITUTE, $encoding, true);
            break;
        case UI_OUTPUT_JS:
            // Solo escapamos caracteres que rompen JS; evitamos double-stripslashes
            $str = addslashes($str);
            break;
        case UI_OUTPUT_RAW:
        default:
    ...
    function checkUserCompanyType () {
        $q  = new DBQuery;
        $q->addTable('companies');
        $q->addQuery('company_type');
        $q->addWhere("company_id = $this->user_company");
        $companyType = $q->loadResult();
        //TODO: Fix this 'burned' var
        $this->fullaccess = ($companyType != 1) ? true : false;
        return $companyType;
    }
}
    ...
    function show() {
        ...
        if (count($this->crumbs) || count($this->cells2)) {
            ...
            foreach ($this->crumbs as $k => $v) {
                ...
                $t .= $AppUI->_($v[0]);
                $crumbs[] = "<a href=\"$k\" class='crumb_link'>$t</a>";
            }
